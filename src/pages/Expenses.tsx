...
import { ArrowDownRight, Filter, Search, MoreVertical, CheckCircle2, Circle, TrendingDown, Calendar, ArrowLeft, ChevronLeft, ChevronRight, Edit2, Trash2, AlertTriangle, X, ArrowUp, ArrowDown, ArrowUpDown, Download, Tag, ChevronDown, FileText, FileX, EyeOff } from 'lucide-react';
...
export const Expenses = () => {
  ...
  const toggleAllSelection = () => {
    if (selectedTransactionIds.length === filteredExpenses.length && filteredExpenses.length > 0) {
      setSelectedTransactionIds([]);
      setIsSelectionMode(false);
          ...
          <div className="flex flex-col md:flex-row items-center justify-end gap-2 w-full md:w-auto order-3 md:order-2">
            <div className="flex items-center gap-2 w-full md:w-auto">
              <div className="relative flex-1 md:w-64">
                ...
                <input
                  type="text"
                  placeholder="Buscar despesa, valor, tag, obs..."
                  title="Busca avançada: descrição, categoria, tags, valor, observação"
                  className="h-9 pl-9 pr-9 rounded-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 w-full transition-shadow duration-200"
                  value={searchTerm}
              ...
              <Button 
                ...
                className={cn(
                  ...
                  isSelectionMode && "bg-[#EE5350]/10 border-[#EE5350] text-[#EE5350]"
                )}
                onClick={() => {
                ...
                <>
                  ...
                  <div className="md:hidden space-y-4">
                    {filteredExpenses.map((t, index) => {
                      ...
                      const isNewDate = index === 0 || t.date !== filteredExpenses[index - 1].date;
                      return (
                        <React.Fragment key={t.id}>
                          ...
                          <div 
                            className={cn(
                              "flex items-center gap-4 p-4 bg-white dark:bg-[#393943] rounded-2xl border mb-4 transition-all",
                              selectedTransactionIds.includes(t.id) ? "border-[#EE5350] bg-[#EE5350]/10" : "border-zinc-100 dark:border-[#393943]"
                            )}
                            onClick={(e) => {
                              if (isSelectionMode) {
